...
func (p *PG) RegisterRoom(c context.Context, x domain.RoomRecord) (domain.RoomRecord, error) {
	e := p.Pool.QueryRow(c, `insert into room(room_id,host,port,protocol_version,gameplay_content_hash,resolved_config_hash,capacity,generation,status,used_players,last_heartbeat,map_id,map_content_version,map_authority_version) values($1,$2,$3,$4,$5,$6,$7,$8,$9,0,now(),$10,$11,$12) on conflict(room_id) do update set host=excluded.host,port=excluded.port,protocol_version=excluded.protocol_version,gameplay_content_hash=excluded.gameplay_content_hash,resolved_config_hash=excluded.resolved_config_hash,capacity=excluded.capacity,generation=excluded.generation,status=excluded.status,last_heartbeat=now(),map_id=excluded.map_id,map_content_version=excluded.map_content_version,map_authority_version=excluded.map_authority_version where room.generation<=excluded.generation returning room_id,host,port,protocol_version,gameplay_content_hash,resolved_config_hash,status,generation,capacity,used_players,last_heartbeat,map_id,map_content_version,map_authority_version`, x.ID, x.Host, x.Port, x.ProtocolVersion, x.GameplayContentHash, x.ResolvedConfigHash, x.Capacity, x.Generation, x.Status, x.MapID, x.MapContentVersion, x.MapAuthorityVersion).Scan(&x.ID, &x.Host, &x.Port, &x.ProtocolVersion, &x.GameplayContentHash, &x.ResolvedConfigHash, &x.Status, &x.Generation, &x.Capacity, &x.UsedPlayers, &x.LastHeartbeat, &x.MapID, &x.MapContentVersion, &x.MapAuthorityVersion)
	return x, e
}
...
func (p *PG) AllocateRoom(c context.Context, player, content string, proto int, config string) (domain.RoomRecord, error) {
	...
	e = tx.QueryRow(c, `select room_id,host,port,protocol_version,gameplay_content_hash,resolved_config_hash,status,generation,capacity,used_players,last_heartbeat,map_id,map_content_version,map_authority_version from room where status='ready' and last_heartbeat>now()-interval '15 seconds' and used_players<capacity and protocol_version=$1 and gameplay_content_hash=$2 and resolved_config_hash=$3 order by room_id for update skip locked limit 1`, proto, content, config).Scan(&r.ID, &r.Host, &r.Port, &r.ProtocolVersion, &r.GameplayContentHash, &r.ResolvedConfigHash, &r.Status, &r.Generation, &r.Capacity, &r.UsedPlayers, &r.LastHeartbeat, &r.MapID, &r.MapContentVersion, &r.MapAuthorityVersion)
	if e != nil {
		return r, e
	}
